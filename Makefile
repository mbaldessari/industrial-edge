...
.PHONY: seed
seed: check-pipeline-resources ## run the seed pipipeline
	oc create -f charts/datacenter/pipelines/extra/seed-run.yaml

.PHONY: build-and-test
build-and-test: ## run a build (all component) and test pipeline
	oc create -f charts/datacenter/pipelines/extra/build-and-test-run.yaml

.PHONY: just-pr
just-pr: ## run a build (iot-consumer/messaging) and test pipeline
